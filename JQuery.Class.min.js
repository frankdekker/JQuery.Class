...
this.bound.start);return this},start:function(a){this.options.preventDefault&&a.preventDefault();this.options.stopPropagation&&a.stopPropagation();var b=this.getPoint(a);this.mouse.start=b;this.trigger("beforeStart",[this.element,b,a]);$(document).on("mousemove touchmove",this.bound.check).on("mouseup touchend touchcancel",this.bound.cancel)},check:function(a){var b=this.getPoint(a);Math.round(Math.sqrt(Math.pow(b.x-this.mouse.start.x,2)+Math.pow(b.y-this.mouse.start.y,2)))<=this.options.snap||($(document).off("mousemove touchmove",
this.bound.check).off("mouseup touchend touchcancel",this.bound.cancel).on("mousemove touchmove",this.bound.drag).on("mouseup touchend touchcancel",this.bound.stop),this.trigger("start",[this.element,b,a]))},drag:function(a){this.trigger("drag",[this.element,this.getPoint(a),this.mouse.start,a])},cancel:function(a){$(document).off("mousemove touchmove",this.bound.check).off("mouseup touchend touchcancel",this.bound.cancel);this.trigger("cancel",[this.element,this.getPoint(a),this.mouse.start,a])},
stop:function(a){$(document).off("mousemove touchmove",this.bound.drag).off("mouseup touchend touchcancel",this.bound.stop);this.trigger("complete",[this.element,this.getPoint(a),this.mouse.start,a])},getPoint:function(a){return a.originalEvent&&a.originalEvent.touches?{x:a.originalEvent.touches[0].pageX,y:a.originalEvent.touches[0].pageY}:a&&a.pageX&&a.pageY?{x:a.pageX,y:a.pageY}:{x:-1,y:-1}}})})();
